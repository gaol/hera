...
readonly HERA_HOSTNAME=${HERA_HOSTNAME:-'podman.host'}
...
copy_artefact_from_parent_job() {
  ...
  echo rsync -ar --exclude hera/ --exclude harmonia/ "${parent_job_dir}" "$(dirname ${workspace})"
  echo -n ' - starting copy at: '
  date +%T
  echo '...'
  rsync -ar --exclude hera/ --exclude harmonia/ "${parent_job_dir}" "$(dirname ${workspace})"
  echo "Done (at $(date +%T))"
